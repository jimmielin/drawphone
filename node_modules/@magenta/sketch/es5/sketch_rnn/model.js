...
var SketchRNN = (function() {
	...
	SketchRNN.prototype.instantiateFromJSON = function(
		info,
		weightDims,
		weightStrings
	) {
		...
		for (var i = 0; i < weightStrings.length; i++) {
			rawWeights = new Float32Array(
				support.stringToArray(weightStrings[i])
			);
		...
		var lstmKernelXH = tf.tensor2d(this.weights[2], [
			this.weightDims[2][0],
			this.weightDims[2][1]
		]);
		var lstmKernelHH = tf.tensor2d(this.weights[3], [
			this.weightDims[3][0],
			this.weightDims[3][1]
		]);
		var axis = 0;
			...
			return __generator(this, function(_a) {
				switch (_a.label) {
					case 0:
						...
						return [
							4,
							fetch(this.checkpointURL).then(function(response) {
	...
	SketchRNN.prototype.getPDF = function(
		state,
		temperature,
		softmaxTemperature
	) {
		...
		var out = tf.tidy(function() {
			...
			var z = tf
				.add(tf.matMul(h, _this.outputKernel), _this.outputBias)
				.squeeze();
